...
def _property_card(row, lang, prefix="s", show_source=False):
    ...
    with st.container():
        ...
        with col_info:
            ...
            s2.markdown(f"🏢 **{row.get('floor',0)}/{row.get('floors_total',0)} {tr('fl',lang)}**" if row.get("floors_total") else "🏢 —")
            s3.markdown(f"🛏️ **{row.get('rooms',0)} {tr('rooms',lang)}**")
            ...
            with st.expander(f"📄 {tr('desc', lang)}"):
                st.write(row.get("description","")[:600])
                agency = row.get("agency","")
                contact = row.get("contact","")
                if contact:
                    st.caption(f"📞 {contact} · {tr('agency',lang)}: {agency}")
                src_url = row.get("source_url","")
                if src_url:
                    st.markdown(f"[{tr('source_lnk',lang)}]({src_url})")
        ...
        with col_price:
            ...
            btn_lbl = tr("watch_rm", lang) if in_wl else tr("watch_add", lang)
            if st.button(btn_lbl, key=f"{prefix}_wl_{prop_id}"):
                if in_wl:
                    remove_from_watchlist(prop_id)
                    st.toast(tr("watch_rm_ok", lang))
                else:
                    ok = add_to_watchlist(dict(row), notes="")
                    st.toast(tr("watch_ok", lang) if ok else tr("watch_dup", lang))
                st.rerun()

            if st.button(tr("mort_btn", lang), key=f"{prefix}_m_{prop_id}"):
...
def render(lang: str = "bg"):
    ...
    with st.sidebar:
        st.markdown(f'<div class="sidebar-box"><h4>📍 {tr("city", lang)}</h4>', unsafe_allow_html=True)
        selected_cities = st.multiselect(
            tr("city", lang), options=sorted(df["city"].unique()),
            default=[], placeholder=tr("city_all", lang), label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(f'<div class="sidebar-box"><h4>🏠 {tr("type", lang)}</h4>', unsafe_allow_html=True)
        type_opts = [(f"{k} / {v}" if lang=="en" else k) for k,v in PROPERTY_TYPES.items()]
        type_sel_d = st.multiselect(tr("type", lang), options=type_opts, default=[], label_visibility="collapsed")
        selected_types = [t.split(" / ")[0] for t in type_sel_d]
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(f'<div class="sidebar-box"><h4>💶 {tr("price", lang)}</h4>', unsafe_allow_html=True)
        price_min, price_max = st.slider(tr("price", lang),
            int(df["price_eur"].min()), int(df["price_eur"].max()),
            (int(df["price_eur"].min()), int(df["price_eur"].max())),
            step=5000, format="€%d", label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(f'<div class="sidebar-box"><h4>📐 {tr("area", lang)}</h4>', unsafe_allow_html=True)
        area_min, area_max = st.slider(tr("area", lang),
            int(df["area_sqm"].min()), int(df["area_sqm"].max()),
            (int(df["area_sqm"].min()), int(df["area_sqm"].max())),
            step=5, format="%d m²", label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(f'<div class="sidebar-box"><h4>🏗️ {tr("const", lang)}</h4>', unsafe_allow_html=True)
        sel_const = st.multiselect(tr("const", lang), CONSTRUCTION_TYPES, default=[], label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(f'<div class="sidebar-box"><h4>⭐ {tr("features", lang)}</h4>', unsafe_allow_html=True)
        req_feats = st.multiselect(tr("features", lang), FEATURES, default=[], label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)

        only_new = st.checkbox(tr("new_only", lang), value=False)
        st.markdown("---")
        sort_opts = [tr("s_pa",lang),tr("s_pd",lang),tr("s_aa",lang),tr("s_ad",lang),tr("s_sqm",lang),tr("s_new",lang)]
        sort_by = st.selectbox(tr("sort", lang), sort_opts)
    ...
    with tab1:
        ...
        sk_map = {tr("s_pa",lang):("price_eur",True), tr("s_pd",lang):("price_eur",False),
                  tr("s_aa",lang):("area_sqm",True),  tr("s_ad",lang):("area_sqm",False),
                  tr("s_sqm",lang):("price_per_sqm",True), tr("s_new",lang):("days_listed",True)}
        sc, asc = sk_map.get(sort_by, ("price_eur", True))
        filtered = filtered.sort_values(sc, ascending=asc)

        c1,c2,c3,c4 = st.columns(4)
        c1.metric(tr("found",lang), len(filtered))
        if len(filtered):
            c2.metric(tr("avg_p",lang), f"€ {filtered['price_eur'].mean():,.0f}")
            c3.metric(tr("avg_a",lang), f"{filtered['area_sqm'].mean():.0f} м²")
            c4.metric(tr("avg_s",lang), f"€ {filtered['price_per_sqm'].mean():.0f}")
        st.markdown("---")

        if len(filtered)==0:
            st.warning(tr("no_res", lang))
        else:
    ...
    with tab2:
        ...
        with lc1:
            live_city = st.selectbox(tr("live_city", lang), list(BULGARIAN_CITIES.keys()), index=0)
        with lc2:
            live_type_opts = list(PROPERTY_TYPES.keys()) if lang=="bg" else [f"{k} / {v}" for k,v in PROPERTY_TYPES.items()]
            live_type_disp = st.selectbox(tr("live_type", lang), live_type_opts)
    ...
    with tab3:
        ...
        if not wl:
            ...
            for item in wl:
                with st.container():
                    ...
                    with wcol2:
                        ...
                        if st.button(tr("wl_remove", lang), key=f"wl_rm_{item['id']}"):
            ...
            st.download_button(tr("wl_export", lang), csv, "watchlist.csv", "text/csv")
